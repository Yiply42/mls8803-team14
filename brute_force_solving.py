...
#TODO: Solve Issue where the EV looks correct, but it recommends all holds when that's clearly not optimal or what the EV is suggesting 
...
for turn in range(4, 0, -1):
            ...
            if turn == 4:
                ...
                for action in actions: #For each possible set of holds/discards we can take
                    ...
                    if (tuple(held_cards), turn) in solved_states:
                        ...
                        if len(held_cards) < 5:
                            solved_states[(tuple(held_cards), turn)] = (best_action, action_ev)
    ...
    print(f"Writing Solutions to {SOLUTIONS_FILE}...")
    with open(SOLUTIONS_FILE, "wb") as f:
        pickle.dump(solved_states, f, protocol=pickle.HIGHEST_PROTOCOL)
